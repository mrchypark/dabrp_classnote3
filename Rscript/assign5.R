...
comp<-fread("./data/recomen/competitor.csv")
...
chen %>% 
  ...
  ggplot(aes(chennel, sum)) + 
  geom_bar(stat = "identity")
chen %>% 
  ggplot(aes(x=chennel,y=useCnt)) + 
  geom_bar(stat="sum")
...
comp %>% 
  group_by(useDate) %>% 
  summarise(N=n()) %>% 
  ggplot(aes(x=useDate, y=N)) + 
  geom_line() + 
  scale_x_date(date_breaks="6 months")

ggplot(comp, aes(x=useDate)) + 
...
comp %>% 
  ...
  summarise(N=n()) %>% 
  ggplot(aes(x=useDate, y=N, color=partner)) + 
  geom_line() + 
  scale_x_date(date_breaks="6 months") + 
  theme(legend.position = "top")

ggplot(comp, aes(x=useDate,color=partner)) + 
  geom_line(stat="count") + 
  scale_x_date(date_breaks="6 months") + 
  theme(legend.position = "top")
