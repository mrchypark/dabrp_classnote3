...
1. 13 페이지의 실습데이터 설정 스크립트를 실행하여 tran.csv(1.4G) 파일을 받으세요
1. 11 페이지의 함수들을 활용하여 데이터를 탐색해보세요    
...
유명하게 사용되는 DBMS는 잘 활용되는 GUI가 있기도 하나 그 기능 구현이 서버와 클라이언트로 되어 있어 연결시 ip 주소가 필요하는 등 관련 상식이 필요함
...
```{r}
conn <- dbConnect(RSQLite::SQLite(), dbname = "../ForderForclass/class2.sqlite")
...
dbWriteTable(conn,
             "member",
             fread("../data/recomen/membership.csv", encoding = "UTF-8"),
             row.names = F,
             overwrite = T
)
```
