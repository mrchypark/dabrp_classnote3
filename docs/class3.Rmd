...
copy_to()는 DBI의 dbWriteTable()과 같은 기능을 수행합니다. dplyr 패키지에 속한 copy_to()는 성능 개선을 통해 dbWriteTable() 보다 빠른 속도를 제공하는 것으로 알려져 있습니다.

```{r}
library(RSQLite)
library(nycflights13)

conn <- dbConnect(SQLite(), dbname = "./test.db")
...
dbplyr과 DBI, dplyr로 데이터베이스의 테이블을 dplyr 문법으로 다루기 위해서는 DBI 패키지에서 conn 객체와 같이 테이블의 연결정보를 담고 있는 R 객체가 필요합니다. tbl()는 DB내 테이블 연결정보를 R 객체로 만드는 함수입니다.
